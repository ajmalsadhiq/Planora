...
const WORKER_BASE = (PUTER_WORKER_URL || "").replace(/\/$/, "");
...
Promise<DesignItem | null | undefined> => {
    ...
    try{
        const response = await puter.workers.exec(`${WORKER_BASE}/api/projects/save`, {
            method:'POST', 
            headers:{
...
export const getProjects = async () => {
    ...
    try {
        const response = await puter.workers.exec(`${WORKER_BASE}/api/projects/list`, {method:'GET'});
...
export const getProjectById = async ({ id }: { id: string }) => {
    ...
    try {
        const response = await puter.workers.exec(
            `${WORKER_BASE}/api/projects/get?id=${encodeURIComponent(id)}`,
            { method: "GET" },
        );
